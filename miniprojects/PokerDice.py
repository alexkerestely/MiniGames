...
def gui():
    ...
    king = 5
    ace = 6
    name= {nine: '9', ten : '10', jack : 'J', queen : 'Q', king : 'K', ace : 'A'}
    ...
    def throws():
        ...
        if len(dice_rerolls) == 0:
            result = 'You finished with ' + hand(dice)
            hand_output.set(result)
        else:
            roll_number = len(dice_rerolls)
            dice_changes = list(range(len(dice_rerolls))) #(1,2..reroll) index
            number_rerolls = roll(roll_number) # generates new cards for rerolls
            iteration = 0
            while iteration < roll_number:
                iteration += 1
                dice_changes[iteration -1] = number_rerolls[iteration - 1] #link a new card to each chosen new index
            iteration = 0
            while iteration < roll_number:
                iteration += 1
    ...
    pd_frame.columnconfigure(0, weight =0)
    pd_frame.rowconfigure(0, weight =0)
    ...
    replay_button = Button(pd_frame, text = 'Replay', command = reset_game).grid(column= 3, row = 7)
